...
export class SeriesPipe<SERIES_IN, SERIES_OUT = SERIES_IN> implements Pipe<SERIES_IN, SERIES_OUT>{
  ...
  /**
   * Start a series, which can start anywhere in the pipelines
   ...
   * case when pipe creates series in middle of pipelines In general, transforms payload out != series out != pipelines payload out
   *
   */
  static start<
...
/*
  transforms<TRANSFORM_CLASS extends Transform<any, any, any>, PASSED_IN = undefined, PIPE_IN = SERIES_IN, PIPE_OUT = SERIES_IN>(transformClass: TransformConstructor<TRANSFORM_CLASS>,
                                                                                                                                passedIn?: PASSED_IN): SeriesPipe<SERIES_IN, SERIES_OUT> {

    // ----- Declaration separator ----- //
    this._pipes.push(TransformPipe.transforms<TRANSFORM_CLASS, PASSED_IN, PIPE_IN, PIPE_OUT>(transformClass, this, passedIn));
    return this;
  };
 ...
 */
  ...
  async execute(rollbackSteps: string[], pipeIn: SERIES_IN): Promise<SERIES_OUT> {
      ...
      for (let i = 0; i < this._transforms.length; i++) {
        try {
          ...
          if(result) {
            ...
            output = await transform.execute(rollbackSteps,_payload, payloadOverride);
            _payload = output;
          } else {
